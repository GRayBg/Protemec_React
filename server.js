...
app.post('/api/compras', upload.single('archivo'), async (req, res) => {
  const { proveedor, costo, categoria, producto, fechaCompra, fecha, urlSaaS } = req.body
  const valorFechaFinal = fechaCompra || fecha
  const file = req.file

  console.log('--- NUEVO REGISTRO (POST) ---')
  console.log('Datos recibidos:', { proveedor, costo, categoria, producto, fechaCompra: valorFechaFinal })
  ...
  try {
    ...
    // Si viene archivo, se procesa y sube a Azure Blob
    if (file) {
...
app.put('/api/compras/:id', upload.single('archivo'), async (req, res) => {
  ...
  console.log('Archivo adjunto:', file ? file.originalname : 'Sin archivo nuevo')

  try {
    let finalUrlSaaS = (urlSaaS && urlSaaS !== 'null' && urlSaaS !== 'undefined') ? urlSaaS : null

    // Si el usuario adjuntó un nuevo archivo durante la edición
    if (file) {
      finalUrlSaaS = await subirABlobYObtenerSAS(file)
    }
